...
for i in range(0,80):
    pred = model(test_set[i][0])
    _,labels_pred = torch.max(pred,dim=1)
    if labels_pred.item() != test_set[i][1].item():
        print('!x')
    print('predict: ',labels_pred.item(),'  real:',test_set[i][1].item())

pred = model(test_set[55][0])
_,labels_pred = torch.max(pred,dim=1)
print('predict: ',labels_pred.item(),'  real:',test_set[55][1].item())
plt.imshow(test_set[55][0].reshape(96,80,-1))
plt.show()
